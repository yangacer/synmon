...
template <typename DirMonitorImplementation = dir_monitor_impl> 
  ...
  public: 
    ...
    { 
      ...
      /*
        ...
        ;
        */
      BOOL res = ReadDirectoryChangesW(ck->handle, ck->buffer, sizeof(ck->buffer), TRUE, filter, &bytes_transferred, &ck->overlapped, NULL); 
      if (!res) 
      { 
        delete ck; 
        DWORD last_error = GetLastError(); 
  ...
  private: 
          ...
          { 
            ...
            { 
              ...
              { 
                ...
                std::string char_fn = to_string(fni->FileName, fni->FileNameLength / sizeof(WCHAR));
                //boost::filesystem::path path(ck->dirname);
                //path /= char_fn;
                //bool is_dir = boost::filesystem::is_directory(path); 
                switch (fni->Action) 
                { 
                ...
                case FILE_ACTION_RENAMED_NEW_NAME:
                  type = dir_monitor_event::added; 
                break; 
                default: type = dir_monitor_event::null;
                break;
                }
                //if( type != dir_monitor_event::null )
                impl->pushback_event(dir_monitor_event(ck->dirname, char_fn, type)); 
    ...
    std::string to_string(WCHAR *filename, DWORD length) 
    { 
      int size = WideCharToMultiByte(CP_ACP, 0, filename, length, NULL, 0, NULL, NULL); 
      if (!size) 
      { 
        DWORD last_error = GetLastError(); 
        boost::system::system_error e(boost::system::error_code(
            last_error, boost::system::get_system_category()), 
          "boost::asio::basic_dir_monitor_service::to_string: WideCharToMultiByte failed"); 
        boost::throw_exception(e); 
      } 
      ...
      { 
        ...
        size = WideCharToMultiByte(CP_ACP, 0, filename, length, dynbuffer.get(), size, NULL, NULL); 
      } 
      else 
      { 
        size = WideCharToMultiByte(CP_ACP, 0, filename, length, buffer, sizeof(buffer), NULL, NULL); 
      } 
